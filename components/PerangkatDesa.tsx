    ...
    <section className="max-w-7xl mx-auto p-6 md:p-10 my-10 text-center">
      <div className="mb-10" data-aos="fade-down">
        <h2 className="text-3xl md:text-4xl font-extrabold text-slate-800 mb-2 tracking-tight">
          Perangkat <span className="text-amber-400">Desa</span>
        </h2>
      ...
      >
        {items && items.map((item: any) => (
          ...
          <SwiperSlide key={item.id}>
            <div className="bg-white p-4 rounded-3xl shadow-md border border-slate-300 hover:border-yellow-200 hover:shadow-xl transition-all duration-500 group">
              <div className="h-80 rounded-2xl overflow-hidden mb-5 relative shadow-inner bg-slate-100">
                ...
                <img 
                  ...
                  alt={item.nama}
                />
                <div className="absolute inset-0 bg-gradient-to-t from-yellow-900/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center pb-6">
                  
                </div>
              </div>
              {/* Nama dan Jabatan langsung panggil kolomnya */}
              <h3 className="font-bold text-slate-800 text-xl tracking-tight">{item.nama}</h3>
              <p className="text-xs text-amber-500 font-black uppercase tracking-[0.15em] mt-2">
                {item.jabatan}
              </p>
            </div>
          </SwiperSlide>
      ...
      <style jsx global>{`
        .swiper-pagination-bullet-active {
          background: #0891b2 !important; /* amber-600 */

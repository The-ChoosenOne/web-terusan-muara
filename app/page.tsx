    ...
    <main className="min-h-screen bg-slate-50 overflow-x-hidden">
      ...
      <section className="py-20 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-black text-slate-900 uppercase tracking-tighter border-b-8 border-amber-400 inline-block pb-2">
            ...
            {allPotensi.slice(0, 3).map((item: any) => (
                <div key={item.id} className="bg-white rounded-[50px] shadow-2xl shadow-slate-900/10 overflow-hidden border border-slate-100 flex flex-col h-full group hover:-translate-y-3 transition duration-500">
                  <div className="h-64 bg-slate-200 relative overflow-hidden">
                    <div className="absolute top-5 left-5 z-10 bg-white/90 backdrop-blur text-amber-600 text-[10px] font-black px-4 py-2 rounded-full shadow-sm uppercase tracking-widest border border-yellow-100">
                  ...
                  <div className="p-10 flex flex-col items-center text-center flex-grow text-black">
                    <h3 className="text-2xl font-black text-slate-800 mb-3 group-hover:text-amber-500 transition leading-tight">{item.nama_potensi}</h3>
                    <p className="text-slate-500 text-sm leading-relaxed mb-8 line-clamp-3 italic font-medium">"{item.deskripsi}"</p>
                    <Link href="/potensi" className="mt-auto w-full bg-amber-300 hover:bg-yellow-600 hover:text-white text-slate-900 font-black py-4 rounded-2xl text-xs transition uppercase tracking-widest shadow-lg active:scale-95 text-center">Lihat Detail</Link>
                  </div>
                </div>
            ))}
          </div>
      ...
      <section className="max-w-6xl mx-auto p-6 md:p-10 mt-10 pb-20">
        <div className="flex justify-between items-center mb-10">
          <h2 className="text-3xl font-black text-slate-900 border-l-8 border-amber-400 pl-4 uppercase tracking-tighter italic">Berita Terkini</h2>
          <Link href="/berita" className="text-amber-500 hover:underline font-black text-sm uppercase tracking-widest">Lihat Semua Berita &rarr;</Link>
        </div>
            ...
            <div key={item.id} className="bg-white rounded-[40px] shadow-lg overflow-hidden flex flex-col h-full border border-slate-100 group hover:shadow-2xl transition duration-500">
              <div className="h-56 bg-slate-200 overflow-hidden relative">
                <img 
                  ...
                  className="w-full h-full object-cover group-hover:scale-110 transition duration-700" 
                />
                <div className="absolute top-4 left-4 bg-slate-900 text-amber-300 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest">Info Desa</div>
              </div>
              <div className="p-8 flex-1 flex flex-col text-black">
                {/* Judul Berita */}
                <h3 className="text-xl font-[1000] mb-3 line-clamp-2 group-hover:text-amber-700 transition leading-tight uppercase italic">{item.judul}</h3>
                ...
                <Link href={`/berita/${item.slug}`} className="mt-auto block text-center py-4 bg-slate-50 text-amber-700 font-black rounded-2xl hover:bg-slate-900 hover:text-amber-300 transition uppercase text-xs tracking-widest shadow-inner">Baca Selengkapnya</Link>
              </div>
            </div>
          ))}
        </div>
